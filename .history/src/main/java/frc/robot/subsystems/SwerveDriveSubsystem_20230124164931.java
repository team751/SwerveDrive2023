...
public class SwerveDriveSubsystem extends SubsystemBase {
  ...
  public SwerveDriveSubsystem(int driveID, int spinID) {
    ...
    drivePidController = new PIDController(Constants.drivePIDDefaultValue, 0, 0);
  ...
  public void drive(SwerveModuleState state) {
    ...
    double motorSpeed = drivePidController.calculate(
        driveMotor.getEncoder().getVelocity() / Constants.motorMaxSpeedMetersPerSecond,
        state.speedMetersPerSecond / Constants.motorMaxSpeedMetersPerSecond);
    driveMotor.set(motorSpeed);
  }
