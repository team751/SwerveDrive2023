...
public class complementaryFilter {
    ...
    public double[] getAngle() {
        deltaTime = System.currentTimeMillis() - olderTime;
        double[] angle = new double[3];
        double[] gyro = sensor.getRotationalVelocity();
        double[] accel = sensor.getAccelerationAngle();
        angle[0] = 0.98 * (anglePrevious[0] + gyro[0] * deltaTime) + percentage * accel[0];
        angle[1] = 0.98 * (anglePrevious[1] + gyro[1] * deltaTime) + percentage * accel[1];
        angle[2] = 0.98 * (anglePrevious[2] + gyro[2] * deltaTime) + percentage * accel[2];
        anglePrevious[0] = angle[0];
        anglePrevious[1] = angle[1];
        anglePrevious[2] = angle[2];
        SmartDashboard.putNumber("filtered", angle[0] + "," + angle[1] + "," + angle[2]);
        olderTime = System.currentTimeMillis();
        return angle;
    }
