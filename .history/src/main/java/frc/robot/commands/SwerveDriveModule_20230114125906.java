...
public class SwerveDriveModule extends CommandBase {
   ...
   */
  public SwerveDriveModule(SwerveDriveSubsystem frontLeft, SwerveDriveSubsystem frontRight,
      SwerveDriveSubsystem backLeft, SwerveDriveSubsystem backRight) {
  ...
  public void initialize() {
    // Set all sparkmax's to follow the front left one
  ...
  @Override
  public void execute() {
    float speed = (float) Constants.driverController.getLeftX() * 0.5f;
    float spin = (float) Constants.driverController.getLeftY() * -0.5f;
    float angle = (float) Constants.driverController.getRightX();
    if (!(speed > 0.05f) && !(speed < -0.1f))
      speed = 0;
    if (!(spin > 0.05f) && !(spin < -0.1f))
      spin = 0;
